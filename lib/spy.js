...
function Spy(context, generatorName) {
  ...
  this.generator = createSpyProxy(this.sourceGenerator, this.calledEventHandler.bind(this)).bind(this.context);
  // set generator to context
  this.setProxyOn();
}
...
Spy.prototype = {
  ...
  setProxyOn: function () {
    if (this.contextIsNotGenerator) {
      this.context[this.generatorName] = this.generator;
    }
  }
};
...
Call.prototype = {
  constructor: Call,
  calledWith: function() { 
    return _.isEqual(this.args, _.toArray(arguments)); 
  }
};

module.exports = function(context, generatorName) { 
  return new Spy(context, generatorName); 
};
